"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
var core_1 = require("@angular/core");
var auth_service_1 = require("../../service/auth.service");
var app_service_1 = require("../../service/app.service");
var app_settings_1 = require("../../app-settings");
var AppNavbarComponent = (function () {
    function AppNavbarComponent(app, authService) {
        this.app = app;
        this.authService = authService;
        this.navCollapsed = true;
        this.isLoggedIn = authService.isLoggedIn();
    }
    AppNavbarComponent.prototype.ngOnInit = function () {
        if (this.navbarId === app_settings_1.AppSettings.APP_NAVBAR_RESPONSIVE) {
            this.navCollapsed = false;
        }
    };
    AppNavbarComponent.prototype.appSidebarToggle = function () {
        this.app.setAppData('sidebarData', { entityLink: false });
        this.app.setAppData('sidebarData', { query: '' });
        this.app.setAppData('sidebarShownQueued', true);
        this.app.sidebarShownQueuedSubject.next(true);
    };
    AppNavbarComponent.prototype.isResponsive = function () {
        if (this.navbarId === app_settings_1.AppSettings.APP_NAVBAR_RESPONSIVE) {
            return true;
        }
        return false;
    };
    AppNavbarComponent.prototype.nullMenu = function () {
        return false;
    };
    return AppNavbarComponent;
}());
__decorate([
    core_1.Input(),
    __metadata("design:type", String)
], AppNavbarComponent.prototype, "navbarId", void 0);
AppNavbarComponent = __decorate([
    core_1.Component({
        selector: 'app-navbar',
        templateUrl: './app-navbar.component.html',
        providers: []
    }),
    __metadata("design:paramtypes", [app_service_1.AppService,
        auth_service_1.AuthService])
], AppNavbarComponent);
exports.AppNavbarComponent = AppNavbarComponent;
//# sourceMappingURL=app-navbar.component.js.map